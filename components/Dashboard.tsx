...
const StatCard: React.FC<{ title: string; value: string; icon: React.ReactNode; color: string; label: string }> = ({ title, value, icon, color, label }) => (
  <div className="bg-[#0a0a0a] p-8 rounded-[24px] border border-white/[0.04] hover:border-white/[0.08] transition-all group flex flex-col justify-between h-48">
    <div className={`w-10 h-10 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center ${color}`}>
      {icon}
    </div>
    <div className="space-y-1 mt-4">
      <div className="text-4xl font-bold tracking-tight text-white">{value}</div>
      <div className="text-[#8a8a8a] text-sm font-medium">{title}</div>
      <div className="text-[10px] font-bold text-indigo-400 mt-2 uppercase tracking-widest">{label}</div>
    </div>
  </div>
);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-1000">
      <header className="space-y-6">
        <p className="text-[#8a8a8a] text-lg font-medium max-w-2xl">
          Your workspace at a glance. Jump into chat, ship tasks, and keep your bot healthy.
        </p>
        <div className="flex items-center gap-3">
          <button className="bg-white text-black px-6 py-3 rounded-full font-bold text-sm flex items-center gap-2 hover:opacity-90 transition-opacity">
            <span>Open Chat</span>
            <ArrowRight size={16} />
          </button>
          <button className="bg-white/[0.05] border border-white/[0.08] text-white px-6 py-3 rounded-full font-bold text-sm hover:bg-white/[0.1] transition-colors">
            View Tasks
          </button>
          <button className="bg-white/[0.05] border border-white/[0.08] text-white px-6 py-3 rounded-full font-bold text-sm hover:bg-white/[0.1] transition-colors">
            Bot Settings
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Leads" value="1,284" label="+42 this week" icon={<Users size={20}/>} color="text-indigo-400" />
        <StatCard title="Tasks" value="64" label="+12 this week" icon={<CheckSquare size={20}/>} color="text-amber-400" />
        <StatCard title="Tables" value="12" label="+0 this week" icon={<Table size={20}/>} color="text-emerald-400" />
        <StatCard title="Documents" value="89" label="+5 this week" icon={<FileText size={20}/>} color="text-cyan-400" />
      </div>
